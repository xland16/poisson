...
function [originalImage,mask,cutTarget,pX,pY,maskedImage,maskedTargetImage] = selectiveCut(source,target) 
...
while 1
  ...
  maskedTargetImage = targetImage;
  for k = 1:size(targetImage,3)
    maskedCutTargetImage = targetImage(pY:pY+m-1,pX:pX+n-1,k);
    maskedCutTargetImage(mask) = 0;
    maskedTargetImage(pY:pY+m-1,pX:pX+n-1,k) = maskedCutTargetImage;
  end
